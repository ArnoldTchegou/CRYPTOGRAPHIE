...
void dechiffrement(uint32_t aes_state[4], uint32_t key[4], uint32_t res_subBytes[4]){
    uint8_t blocks;
    uint32_t Resultat[4] = {0};
    printf("Resultat avant le dechiffrement=============\n\n");
    print_state(aes_state);
    for (int i = 0; i < 4; i++){
    	for(int j = 3; j >= 0; j--){
    		unit8_extractBlocks(&res_subBytes[i], &blocks,j);
    		int k = 0;
    		while(blocks != SBox[k]){
    			k++;
    		}
    		Resultat[i] |= (uint32_t)k<< (j * 8);	
    	}
    }
	for(int i = 0; i < 4; i++){
		key[i] =  aes_state[i]^Resultat[i];
	}
	printf("Resultat de la clé de chiffrement=============\n\n");
	print_state(key);
}

int main(){
	uint32_t key[4];
	dechiffrement(aes_state, key, res_subBytes);
}
