...
//utilisée pour extraire la round_key à chaque tour
void extract_Round_key(uint32_t* Round_key, uint32_t aes128_extended_key[4*11], int j, int k){
	for(int i=0; i<4; i++){
		*Round_key = ((aes128_extended_key[4*k + i] >> (j * 8)) & 0xFF)|(*Round_key<<8);
	}
}
...
void AddRoundKey(uint32_t aes_state[4], uint32_t aes128_extended_key[4*11], int k){
	uint32_t Round_key;
	for(int i=0; i<4; i++){
		extract_Round_key(&Round_key, aes128_extended_key, 3-i, k);
		aes_state[i] = aes_state[i]^Round_key;
	}
}
...
int main(){
	...
	//printf("Before AES encryption=============\n\n");
	//print_state(aes_state);
	//AES_encryption(aes_state, aes128_extended_key);
	//printf("\n");
	...
	AddRoundKey(message, aes128_extended_key, 0);
	SubBytes(message);
	ShiftRows(message);
	print_state(message);
}
