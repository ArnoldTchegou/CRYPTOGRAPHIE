...
void Block_Cipher(uint32_t IV[4], uint32_t aes_state[4], uint32_t aes128_extended_key[4*11]);
void add_init_vector(uint32_t IV[4], uint32_t aes_state[4]);
void aes_cbc(uint32_t IV[4],  uint32_t* message, int m_len, uint32_t aes128_extended_key[4*11]);
...
void InvBlock_Cipher(uint32_t IV[4], uint32_t aes_state[4], uint32_t aes128_extended_key[4*11]);
void copy(int n, uint32_t aes_state[n], uint32_t aes_state_copy[n]);
void aes_cbc_dechiffrement(uint32_t IV[4], uint32_t* message, int m_len, uint32_t aes128_extended_key[4*11]);
...
int main(){
	...
	printf("message\n");
	print_state(12, message);
	aes_cbc(IV, message, 384, aes128_extended_key);
	...
	aes_cbc_dechiffrement(IV1, message, 384,aes128_extended_key);
	printf("message dechifré\n");
	print_state(12, message);	
}
...
void aes_cbc(uint32_t IV[4], uint32_t* message, int m_len, uint32_t aes128_extended_key[4*11]){
	...
	int m = m_len/128;
		for(int i=0; i<m; i++){
			...
			tab[2] = *(message + 4*i + 2);
			tab[3] = *(message + 4*i + 3);
			Block_Cipher(IV, tab, aes128_extended_key);
			*(message + 4*i) = tab[0];
			*(message + 4*i + 1) = tab[1];
			*(message + 4*i + 2) = tab[2];
			*(message + 4*i + 3) = tab[3];
			//re-initialiser le vecteur IV
			state_copy(message+4*i, IV);	
		}
}
...
//DECHIFFREMENT AES_CBC
void aes_cbc_dechiffrement(uint32_t IV[4], uint32_t* message, int m_len, uint32_t aes128_extended_key[4*11]){
	uint32_t tab[4]={0,0,0,0};
	int m = m_len/128;
	uint32_t message_chiff[4*m];
	copy(4*m, message, message_chiff);
		m = m_len/128;
		for(int i=0; i<m; i++){
			tab[0] = *(message + 4*i);
			tab[1] = *(message + 4*i + 1);
			tab[2] = *(message + 4*i + 2);
			tab[3] = *(message + 4*i + 3);
			InvBlock_Cipher(IV, tab, aes128_extended_key);
			*(message + 4*i) = tab[0];
			*(message + 4*i + 1) = tab[1];
			*(message + 4*i + 2) = tab[2];
			*(message + 4*i + 3) = tab[3];
			//re-initialiser le vecteur IV
			state_copy(message_chiff+4*i, IV);	
		}		
}
